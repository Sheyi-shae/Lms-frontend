...
export default function DashboardDetails() {
  ...
  return (
    ...
    <Card className="col-span-4 border-l-4 border-l-cyan-600">
      <CardHeader>
        ...
        <div key={item.id|| i} >
          <div className="flex items-center">
            <div className="flex items-center gap-2">
              ...
              <div>
                <p className="text-sm font-medium leading-none line-clamp-1">{item.course.title}</p>
                <Link href={`/instructor/coursedetails/${item.course.id}`}>
                <p className="text-sm text-muted-foreground line-clamp-1">{item.student.name}</p>
                </Link>
              </div>
            </div>
    ...
    <Card className="col-span-4 md:col-span-3 lg:col-span-3">
      <CardHeader>
        ...
        <CardDescription>Recently enrolled courses this month</CardDescription>
      </CardHeader>
      <CardContent>
        {loading ? (
          <div className="flex items-center justify-center h-32">
        ...
        <div className="space-y-6">
            {courses.slice(0,5).map((course) => ( 
                ...
                <Link href={`/instructor/coursedetails/${course.id}`} >
                    <div  className="flex items-center">
                    <div className="space-y-1">
                        <p className="text-sm font-medium leading-none">{course.title}</p>
                        <p className="text-sm text-muted-foreground">{course.enrollments.length} students</p>
                    </div>
